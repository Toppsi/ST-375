...
I dette projekt anvendes en ADC af typen NI USB-6009 til at konvertere det analoge signal til digital, jævnfør afsnit \ref{ADCafsnit} på side \pageref{ADCafsnit}. %Signalet skal konverteres, for at kunne sendes ind i den digitale del af systemet, så plejepersonalet kan aflæse og behandle målingerne. 
Med denne ADC kan der samples med $13$ bits single-ended. ADC'en kan dermed inddeles i $2^{13} = 8192 \text{niveauer}$, jævnfør afsnit \ref{ADC_afsnit} på side \pageref{ADC_afsnit}. Den maksimale sampling rate er på $48$kS/s, hvormed det er muligt at sample med minimum det tidobbbelte af båndbredden. Arbejdsområdet for ADC'en ift. spænding ligger på $10Vpp$ og har en typisk præcision på $14.7$mV ved $25^{\circ}$C. \cite{Instruments2014} Ud fra oplysningerne udregnes LSB til $2.44$mV ved brug af ligningen i afsnit \ref{ADCafsnit} på side \pageref{ADCafsnit} %er det muligt at udregne LSB vha. ligningen i afsnit \ref{ADCafsnit} på side \pageref{ADCafsnit}. Udregningen ser således ud: \\
%\begin{equation}
%	LSB = \frac{FSR}{2^{n}} 
%\end{equation}  
...
\end{enumerate}
Der samples med $500$Hz, selvom $250$Hz havde været nok, da vores båndbredde er $25$Hz, men ScopeLogger kan enten sample med $200$ eller $500$, så derfor samples med $500$Hz. På \figref{fig:ADC_Test} ses resultatet af målingerne.
